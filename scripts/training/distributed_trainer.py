...
class HiggsAudioDistributedTrainer:
    ...
    def create_dataloaders(self, train_path, val_path):
        ...
        self.collator = HiggsAudioSampleCollator(
            ...
            audio_in_token_id=tokenizer.convert_tokens_to_ids("<|AUDIO|>"),  # Fixed: added pipes
            audio_out_token_id=tokenizer.convert_tokens_to_ids("<|AUDIO_OUT|>"),  # Fixed: added pipes
            pad_token_id=tokenizer.pad_token_id,
            audio_stream_bos_id=tokenizer.convert_tokens_to_ids("<|audio_stream_bos|>"),  # Fixed: added pipes
            audio_stream_eos_id=tokenizer.convert_tokens_to_ids("<|audio_stream_eos|>"),  # Fixed: added pipes
            round_to=8,
            pad_left=False,
    ...
    def _inference_style_collate_fn(self, batch):
        ...
        for item in batch:
            ...
            # This function ALREADY inserts audio tokens where needed
            input_tokens, label_tokens, audio_contents, speaker_id = prepare_chatml_sample(
                chatml_dict, self.text_tokenizer
            )
            
            # Load reference audio waveforms and tokenize them
            reference_waveforms = []
            reference_audio_ids = []  # Store tokenized audio codes
                ...
                if audio_content and hasattr(audio_content, 'audio_url'):
                    ...
                    if audio_path and os.path.exists(audio_path):
                        try:
                            ...
                            # Tokenize the reference audio to get audio codes
                            # This is CRITICAL for zero-shot voice cloning
                            if self.audio_tokenizer_ref is not None:
                                audio_tokens = self.audio_tokenizer_ref.encode(audio_path)
                                # Ensure we only use the first 8 codebooks
                                if audio_tokens.shape[0] > 8:
                                    audio_tokens = audio_tokens[:8, :]
                                reference_audio_ids.append(audio_tokens)
                                self.logger.debug(f"Tokenized reference audio: {audio_tokens.shape}")
                            
                        except Exception as e:
                            self.logger.warning(f"Failed to load reference audio {audio_path}: {e}")
            
            # Prepare audio_ids_concat and audio_ids_start (CRITICAL for collator)
            if reference_audio_ids:
                # Concatenate all audio tokens
                audio_ids_concat = torch.cat(reference_audio_ids, dim=1)  # Shape: (8, total_tokens)
                # Calculate start indices for each audio segment
                audio_ids_start = torch.cumsum(
                    torch.tensor([0] + [ids.shape[1] for ids in reference_audio_ids], dtype=torch.long), 
                    dim=0
                )
            else:
                # No reference audio - use empty tensors
                audio_ids_concat = torch.empty((8, 0), dtype=torch.long)
                audio_ids_start = torch.tensor([], dtype=torch.long)
            
            # Calculate waveform start indices
            if reference_waveforms:
                waveform_lengths = [wv.shape[1] for wv in reference_waveforms]
                audio_waveforms_start = torch.cumsum(
                    torch.tensor([0] + waveform_lengths[:-1], dtype=torch.long),
                    dim=0
                )
            else:
                audio_waveforms_start = torch.tensor([], dtype=torch.long)
            
            # Create ChatMLDatasetSample with properly tokenized reference audio
            # This matches the inference pipeline exactly
            chatml_sample = ChatMLDatasetSample(
                input_ids=torch.tensor(input_tokens, dtype=torch.long),
                label_ids=torch.tensor(label_tokens, dtype=torch.long),
                audio_ids_concat=audio_ids_concat,  # Contains tokenized audio codes
                audio_ids_start=audio_ids_start,    # Contains proper indices
                ...
                audio_sample_rate=torch.tensor([24000] * len(reference_waveforms) if reference_waveforms else [], dtype=torch.float32),
                audio_speaker_indices=torch.tensor([speaker_id if speaker_id is not None else 0] * len(reference_waveforms) if reference_waveforms else [], dtype=torch.long)
            )
