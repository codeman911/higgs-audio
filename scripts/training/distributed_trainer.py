...
class HiggsAudioDistributedTrainer:
    ...
    def _inference_style_collate_fn(self, batch):
        ...
        for item in batch:
            ...
            # Check if audio tokens are already present in input_tokens
            input_tensor = torch.tensor(input_tokens, dtype=torch.long)
            existing_audio_mask = (input_tensor == audio_in_token_id)
            num_existing_audio_tokens = existing_audio_mask.sum().item()
            
            # Only insert audio tokens if:
            # 1. We have reference audio waveforms
            # 2. Audio tokens are not already present (or fewer than expected)
            if reference_waveforms and num_existing_audio_tokens < len(reference_waveforms):
                # Calculate how many audio tokens to insert
                tokens_to_insert = len(reference_waveforms) - num_existing_audio_tokens
                ...
                if tokens_to_insert > 0:
                    ...
                    # Add missing audio tokens for reference audio at the beginning
            ...
            # Ensure input_ids is always a tensor (never empty list)
            if not input_tokens:
                # Fallback: create minimal valid tokens if somehow empty
                input_tokens = [self.text_tokenizer.bos_token_id] if hasattr(self.text_tokenizer, 'bos_token_id') else [1]
                label_tokens = [-100]
