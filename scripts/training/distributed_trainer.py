...
def main():
    ...
    def prepare_supervision(batch, tokenizer, device, logger=None):
        ...
        return dict(
            ...
            # For 8-codebook system, each audio segment gets 8 streams, but they all have the same length
            audio_in_ids_start=torch.tensor([0, a_in.shape[1]] if a_in is not None else [0], dtype=torch.long, device=device),
            audio_out_ids_start=torch.tensor([0, a_shift.shape[1]] if a_shift is not None else [0], dtype=torch.long, device=device),
