...
class HiggsAudioDistributedTrainer:
    ...
    def _inference_style_collate_fn(self, batch):
        """
        Collate function that matches inference pipeline exactly.
        ...
        for item in batch:
            ...
            # Load reference audio waveforms
            reference_waveforms = []
            for audio_content in audio_contents:
                if audio_content and 'audio_url' in audio_content:
            ...
            chatml_sample = ChatMLDatasetSample(
                ...
                audio_speaker_indices=torch.tensor([0] if reference_waveforms else [], dtype=torch.long)  # Default speaker ID for data without speaker info
            )
