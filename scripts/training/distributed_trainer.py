    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch + 1}")):
                ...
                if step == 0:
                    self.logger.info(f"Original batch keys: {list(batch.keys())}")
                    
                    # Extract labels before removing them from batch
                    labels = batch.get('label_ids')
                    audio_labels = batch.get('label_audio_ids')
                    
                    # Create clean model inputs (remove labels)
                    model_inputs = {k: v for k, v in batch.items() 
                                  if k not in ['label_ids', 'label_audio_ids']}
                    
                    self.logger.info(f"Model input keys (no labels): {list(model_inputs.keys())}")
                ...
                with self.accelerator.accumulate(model):
                    try:
                        outputs = model.forward(
                            ...
                            audio_out_ids_start_group_loc=model_inputs.get('audio_out_ids_start_group_loc'),
                            reward=model_inputs.get('reward')
                        )
                        ...
                        self.logger.error(f"Batch keys: {list(batch.keys())}")
