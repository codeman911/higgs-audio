#!/usr/bin/env python3
"""
Unified LoRA training script for Higgs Audio model with two-stage training support.

Stage 1: Text-only training with audio components frozen
Stage 2: Full LoRA training with all components trainable
"""
...
def freeze_audio_components(model):
    """Freeze all audio-specific components in the model for stage 1 training."""
    logger.info("Freezing audio components for stage 1 training...")
    ...
    for name, param in model.named_parameters():
        ...
        # Freeze audio-specific components
        if any(audio_component in name for audio_component in [
            'audio_mlp',           # Audio MLP layers
            'audio_attn',          # Audio attention modules
            'audio_input_layernorm',
            'audio_post_attention_layernorm', 
            'audio_post_audio_attn_layer_norm',
    ...
    logger.info(f"Frozen {frozen_params}/{total_params} parameters ({100*frozen_params/total_params:.2f}%)")
    return model


def get_text_only_lora_targets(model):
    """Get LoRA target modules for text-only training (exclude audio components)."""
    target_modules = []
    
    # Scan model for text-only modules
    for name, module in model.named_modules():
        # Target standard attention projections (text only)
        if any(proj in name for proj in ["q_proj", "k_proj", "v_proj", "o_proj"]) and "audio" not in name:
            target_modules.append(name)
        
        # Target standard MLP layers (text only)
        if any(mlp in name for mlp in ["gate_proj", "up_proj", "down_proj"]) and "audio" not in name:
            target_modules.append(name)
...
class HiggsAudioTrainer:
    """Minimal trainer for DualFFN LoRA fine-tuning with stage support."""
    ...
    def load_model_and_tokenizers(self):
        ...
        if self.args.training_stage == "stage1":
            ...
            self.model = freeze_audio_components(self.model)
            
            # Apply LoRA with text-only targets
            text_targets = get_text_only_lora_targets(self.model)
            ...
            logger.info(f"Stage 1: Applying LoRA to {len(text_targets)} text modules")
        else:
            # Stage 2: Full LoRA training
            lora_config = create_lora_config(
                r=self.args.lora_r,
                lora_alpha=self.args.lora_alpha,
                lora_dropout=self.args.lora_dropout
            )
            logger.info("Stage 2: Applying LoRA to all modules")
    ...
    def _compute_dual_loss(self, outputs, text_labels, audio_labels):
        ...
        # For stage 1, we ignore audio loss since audio components are frozen
        if self.args.training_stage == "stage1":
            loss_dict['audio_loss'] = 0.0
        elif audio_logits is not None and audio_labels is not None:
        ...
        if self.global_step % self.args.log_steps == 0 and self.local_rank == 0:
            ...
            # Log audio predictions if we have audio logits (and not in stage 1)
            if self.args.training_stage != "stage1" and audio_logits is not None and audio_labels is not None and audio_logits.numel() > 0:
    ...
    def train(self):
        ...
        if dist.get_rank() == 0:
            ...
            logger.info("=" * 60)
            logger.info(f"Training Stage: {self.args.training_stage}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Higgs-Audio LoRA Training with Stage Support")
    ...
    # Stage argument
    parser.add_argument("--training_stage", type=str, choices=["stage1", "stage2"], required=True,
                        help="Training stage: 'stage1' for text-only, 'stage2' for full training")
